...
def transform_newrelic_conf():
    ...
    content = (
        template.replace('__LICENSE_KEY__', subprocess.Popen(newrelic_license_key.split(" "), stdout=subprocess.PIPE,  shell=True).stdout.readline())                
            .replace('__ENVIRONMENT_NAME__', '{}'.format(subprocess.Popen(env.split(" "), stdout=subprocess.PIPE,  shell=True).stdout.readline()))
            .replace('__NEWRELIC_NAME__', '{}'.format(subprocess.Popen(app_name.split(" "), stdout=subprocess.PIPE, shell=True).stdout.readline()))
